...
# ==========================================
# 2. CÁC HÀM TÍNH TOÁN CHỈ SỐ BẰNG LLM
# ==========================================
...
def calculate_fact_score(question, reference, candidate):
    try:
        extract_prompt = ChatPromptTemplate.from_template(
            "Dựa trên câu trả lời sau, hãy liệt kê từng tuyên bố sự thật ngắn gọn dưới dạng gạch đầu dòng.\n"
            "Câu trả lời: {candidate}\n"
            "CHỈ liệt kê các sự thật, không giải thích thêm."
        )
        extract_chain = extract_prompt | judge_llm | parser
        facts_raw = extract_chain.invoke({"candidate": candidate})
        ...
        verify_prompt = ChatPromptTemplate.from_template(
            ...
            "Dựa trên đáp án chuẩn, hãy kiểm tra tuyên bố sau là ĐÚNG hay SAI: {fact}\n"
...
cli_parser = argparse.ArgumentParser(description="Đánh giá answers_agentic, có thể chỉ định id câu hỏi.")
cli_parser.add_argument("--ids", type=parse_id_selection, help="Danh sách id cần đánh giá, ví dụ: 1 hoặc 1,3,5-7")
cli_parser.add_argument("--answers", default=os.path.join(base_dir, 'answers_agentic.json'), help="File câu trả lời cần đánh giá.")
...
# ==========================================
# 4. CHẠY ĐÁNH GIÁ (THÊM AUTOMATIC RECALL)
# ==========================================
# Căn chỉnh lại header để hiển thị thêm cột Auto-Rec
print(f"\n{'ID':<3} | {'Ans-Sim':<7} | {'Auto-Pre':<8} | {'Auto-Rec':<8} | {'LLM-Rel':<7} | {'LLM-Rec':<7} | {'Faithful':<8} | {'Fact':<5} | {'LLM-Pre':<7}")
print("-" * 84)
...
for item in answers_data:
    ...
    # Nhóm 1: Semantic / Automatic Evaluation
    # 1.1 Answer Similarity (So sánh Bot Answer và Ground Truth)
    v_bot = embeddings.embed_query(bot_answer)
    v_gt = embeddings.embed_query(ground_truth)
    sem_s = cosine_similarity([v_bot], [v_gt])[0][0]

    # 1.2 & 1.3 Semantic Context Precision & Context Recall
    ...
    if contexts:
        ...
        # Tính Semantic Precision: So sánh Câu hỏi với Contexts
        q_ctx_similarities = [cosine_similarity([v_q], [vc])[0][0] for vc in v_ctx]
        auto_pre_scores = []
        for i, sim in enumerate(q_ctx_similarities):
            if sim > 0.4:  # Ngưỡng liên quan
                relevant_count = len([s for s in q_ctx_similarities[:i+1] if s > 0.4])
                auto_pre_scores.append(relevant_count / (i + 1))
        auto_precision = np.mean(auto_pre_scores) if auto_pre_scores else 0.0
        
        # Tính Semantic Recall: So sánh Đáp án chuẩn (Ground Truth) với Contexts
        gt_ctx_similarities = [cosine_similarity([v_gt], [vc])[0][0] for vc in v_ctx]
        # Nếu đoạn ngữ cảnh tốt nhất chứa thông tin cực kỳ giống đáp án chuẩn -> Recall cao
        auto_recall = max(gt_ctx_similarities) if gt_ctx_similarities else 0.0

    # Nhóm 2: LLM-Judged Evaluation
    ...
    c_recall = get_llm_score(
        recall_template,
        question=question,
        ground_truth=ground_truth,
        context_text=context_text,
    ) if contexts else 0.0
    faithfulness = get_llm_score(
        faithfulness_template,
        question=question,
        context_text=context_text,
        bot_answer=bot_answer,
    ) if contexts else 0.0
    fact_score = calculate_fact_score(question, ground_truth, bot_answer)
    llm_precision = calculate_llm_context_precision(question, contexts) if contexts else 0.0

    print(f"{q_id:<3} | {sem_s:<7.2f} | {auto_precision:<8.2f} | {auto_recall:<8.2f} | {c_relevance:<7.2f} | {c_recall:<7.2f} | {faithfulness:<8.2f} | {fact_score:<5.2f} | {llm_precision:<7.2f}")
    
    # Cập nhật danh sách lưu trữ để tính trung bình
    results_summary.append([sem_s, auto_precision, auto_recall, c_relevance, c_recall, faithfulness, fact_score, llm_precision])
...
if results_summary:
    ...
    print("-" * 84)
    print(f"{'AVG':<3} | {avgs[0]:<7.2f} | {avgs[1]:<8.2f} | {avgs[2]:<8.2f} | {avgs[3]:<7.2f} | {avgs[4]:<7.2f} | {avgs[5]:<8.2f} | {avgs[6]:<5.2f} | {avgs[7]:<7.2f}")
